...
그래서 **핸들 발급기는 shim 모듈 전체에 하나**다 (`allocateHandle`). 네이티브 레지스트리는
컨텍스트당 하나이고 핸들 정수만으로 객체를 찾으므로 — 디바이스별 칸이 없다 — 카운터를
디바이스마다 두면 두 번째 디바이스가 1번부터 다시 발급해 **첫 디바이스의 객체를 조용히
덮어쓴다.** 오류가 나지 않고 "내 버퍼에 남이 그리는" 증상만 남는 종류다. 레지스트리 쪽에도
겹침을 세는 카운터(`displacedHandleCount`)를 두어, 그런 일이 생기면 로그에 원인이 남는다.
...
present 없이 끝난 프레임(첫 인코더 전에 검증 오류로 죽은 프레임 등)의 드로어블은 **그 캔버스가
다음에 다시 획득할 때** 회수된다 — 배치 끝마다 회수하면 위의 "프레임 진행 중" 상태를 부수기
때문이다. 이 회수는 **캔버스 단위**다: 다른 캔버스가 프레임 중간이어도 그쪽 드로어블과 핸들은
건드리지 않는다. 캔버스가 프레임 도중 사라지면(`detachCanvas` — 엘리먼트 deinit) 다시 획득할
기회 자체가 없으므로, **해제 시점에 그 캔버스의 획득분을 즉시 회수한다** — 남겨 두면 드로어블이
영영 풀로 돌아가지 못하고, 다음 present가 그리지도 않은 프레임을 죽은 레이어에 내보낸다.
