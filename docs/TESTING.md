...
| **명세 발산** — 씬이 관대한 Metal 런타임에 기대던 것 (5) | wgsl·hdr — varying 분기 안 `textureSample` (uniformity 위반; 샘플을 분기 앞으로 올리면 해소됨을 확인, 씬은 무변경) · stencil·query — `layout:"auto"` 파이프라인의 바인드 그룹을 **다른** 파이프라인에 재사용 (명세상 auto 레이아웃은 파이프라인 전용 — 모든 드로우가 거부되어 캔버스는 비고, 오류는 오버레이에 표시된다) · images·**contracts** — `copyTextureToBuffer`의 `bytesPerRow` 32 (256 배수 위반 — images는 리드백 1건만 실패하고 그림은 그려지고, contracts는 vec3 검사 판독이 0으로 읽혀 8/10이 된다). contracts의 나머지 실패 1건은 **번들 내부 검증 시점**이다 — 명세(와 Dawn)는 번들 `finish()` 시점에 "Index buffer was not set"을 내지만, Metal의 엔진 재생은 실행 시점에 낸다. 격리 자체는 양쪽 다 지켜진다 |
...
> contracts는 8/10 (남은 2건은 위 발산 표 참고).
...
| 프레임 경계 | `JS/tests/frame-boundary.test.mjs` | present는 **틱의 끝**에 한 번(명세 "updating the rendering") · 마무리 배치는 명령이 없어도 나감 · 작업 없는 틱은 브리지 미통과 · 루프 밖 submit은 그 자리에서 present · 콜백이 던져도 present는 나가고 오류는 안 삼킴 · **티커 참조 계수**(남의 stop에 안 끌려감) · `getCurrentTexture`는 만료 전까지 **같은 텍스처**(present·configure·리사이즈에서 만료, 설정 전엔 `InvalidStateError`) |
...
| 렌더 번들 | `RenderBundleTests` | **직접 인코딩과 프레임 전체 동치성**, 재사용(두 프레임 연속), 실행 순서, **상태 격리 양방향**(파이프라인·바인드 그룹·정점 버퍼·인덱스 버퍼 네 가지 다), **실행 중 오류가 나도 격리가 성립하는지**, 포맷·어태치먼트 수 불일치, **후행 `null` 무시**, **어태치먼트 최소 하나**, **깊이 전용 MSAA 패스의 `sampleCount`**, **`depthReadOnly` 패스에는 readOnly 번들만**, 번들에 금지된 명령, **하나만 비호환이어도 앞의 호환 번들까지 미실행**, **디버그 마커는 담을 수 있다**(명세가 `GPUDebugCommandsMixin`을 포함한다 — 빠뜨리면 마커 하나로 번들 전체가 거부된다) |
