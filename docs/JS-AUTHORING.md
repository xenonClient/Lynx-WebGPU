...
바이트열은 **`ArrayBuffer`로 그대로** 브리지를 건넌다 (Lynx가 `NSData`로 바꿔 준다). 문자열 인코딩이
없으므로 팽창도 인코딩 루프도 없다 — `writeBuffer`/`writeTexture`/`mapAsync` 모두 같다.
`writeTexture`/`writeBuffer`는 GPU 완주를 기다리지 않고 큐 순서를 타므로, 수십 KB급 동적
텍스처(예: 128×128 RGBA = 64KB)를 매 프레임 갱신해도 된다 (데모의 `dynamic` 씬).
그래도 매 프레임 수 MB를 올린다면 브리지 왕복보다 스토리지 버퍼 + 컴퓨트 셰이더로 GPU 안에서
갱신하는 편이 낫다.
