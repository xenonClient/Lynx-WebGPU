...
| 3 | 트랜스파일러 코퍼스 (§3) | — | **코어 WGSL은 숙제 없음** — 91%, 남은 2건은 호스트 생성 코드와 WGSL 확장(`buffer_view`)이다 |
...
| **Dawn + `threelab`의 비교 샘플러** | — | **미해결.** 시뮬레이터에서 Dawn이 비교 샘플러를 거부해(Metal family 2 보고) three.js의 그림자 바인드 그룹이 통째로 무효가 되고 캔버스가 빈다. 같은 시뮬레이터에서 **Metal 런타임은 11/11로 그린다**. 실기기(A12+)에서 풀릴 것으로 보지만 확인 전까지 열어 둔다 (`docs/TESTING.md` §2-1) |
...
`webgpu-samples` 완성 모듈 67개 중 **61개 통과(91%)** (2026-08-09 재측정 — 그대로 60 + 조립 1).
이전에 "실패 3건"으로 세던 것은 **하나가 진짜 버그, 둘은 측정 방식 문제**였다:
...
| `skinnedMesh/gltf.wgsl` | 호스트가 glTF 접근자를 보고 `VertexInput`을 **런타임 생성**해 붙인다 | **남은 2건 중 하나.** 파일만으로는 완성될 수 없다 — 지원 대상이 아니라고 보는 것이 맞다 |
| `wireframe/wireframeBufferView.wgsl` | `requires buffer_view;`로 **WGSL 확장**을 켜고 `bufferView<vec4u>(&buf, …)`를 쓴다 — 코어 명세 밖의 문법이다 | **남은 2건 중 하나.** 파서 버그가 아니라 **미구현 확장**이다 — 열려면 `requires` 구문과 `bufferView<T>()`를 함께 넣어야 한다 |

즉 **코어 WGSL 쪽 숙제는 없고**, 남은 하나는 확장을 구현할지 말지의 선택이다.
통과율을 더 올리려면 코퍼스를 늘리는 편이 낫다
...
limits 노출은 **이미 끝났다**(위 표의 첫 행). 남은 기준은 색공간·업로드·중간 텍스처 셋이고,
그 결과로 `JS-AUTHORING.md`에 "큰 이미지를 다룰 때" 항목이 생긴다.
