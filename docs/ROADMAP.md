...
남은 것은 three가 캔버스 뷰를 **프레임을 넘겨** 재사용하는 자리다 (`docs/WEBGPU-API.md` §8-1).
명세대로면 present에서 만료된 뷰라 거부가 맞고, 우회는 렌더 타깃에 받아 마지막에 옮기는 것이다.

---

## 포스트프로세싱 캔버스 출력 — 남은 자리

`threelab` 데모 씬이 찾아낸 것이다 (`docs/WEBGPU-API.md` §8-1). three의 `PostProcessing`을
캔버스로 직접 내보내면 드로어블 뷰 캐시가 우리 present 시점과 어긋난다.

프레임 경계는 위에서 옮겼다. 남은 것은 three의 `_getRenderPassDescriptor()`가 캔버스 뷰를
**프레임을 넘겨** 캐시하는 자리다 — 명세대로면 present에서 만료된 뷰이므로 거부가 맞다.

여기를 더 밀려면 three 쪽을 봐야 한다 (캔버스 텍스처에 `externalTexture` 표시를 붙여
디스크립터를 캐시하지 않게 하는 것이 three의 기존 장치다). 우회는 렌더 타깃에 받아 마지막에
한 번만 캔버스로 옮기는 것이고, `threelab` 합성 화면이 그 구성으로 60fps로 돈다.
