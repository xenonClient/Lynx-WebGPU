...
final class WGPUFrameCoordinatorTests: XCTestCase {
    ...
    func test_한도에_닿으면_준비_안됨이_되고_완료가_돌아오면_풀린다() {
    ...
    func test_추적하지_않는_캔버스는_항상_준비_상태다() {
        let coordinator = WGPUFrameCoordinator()
    ...
    func test_하나라도_포화되면_전체가_준비_안됨이다() {
        let coordinator = WGPUFrameCoordinator()
    ...
    func test_해제한_캔버스는_더_이상_틱을_막지_않는다() {
        let coordinator = WGPUFrameCoordinator()
    ...
    func test_동시_통지에도_카운터가_어긋나지_않는다() {
    ...
    func test_명령이_없는_present_배치는_틱의_마무리다() {
    ...
    func test_명령이_있는_present_배치는_마무리_배치가_아니다() {
