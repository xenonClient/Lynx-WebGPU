...
/// 크래시 하드닝 검증 — "잘못된 인자로 프로세스를 죽이지 않는다"(`WGPUError`)가 **Metal
/// 런타임에도** 성립하는가. JS는 어떤 정수든 실어 보낼 수 있으므로, 음수·거대값·NaN이 GPU
/// 인자 폭 변환에서 **트랩(프로세스 종료)이 아니라 validation 오류**가 되어야 한다.
///
/// Metal에서 특히 중요하다 — Metal 검증 레이어는 잘못된 인자에 `assert`로 **프로세스를 죽인다.**
/// Dawn은 오류를 돌려주는 쪽이라, 같은 입력이라도 죽는 쪽은 이쪽이다.
/// `DawnHardeningTests`와 **같은 입력**을 쓴다 — 두 백엔드가 같은 계약을 지키는지 보려면 그래야 한다.
