...
/// Crash hardening — whether "bad arguments never kill the process" (`WGPUError`) holds **on the Metal
...
///
/// It matters especially on Metal — the Metal validation layer **kills the process** with an `assert`
/// on a bad argument. Dawn returns an error instead, so on identical input this is the side that dies.
/// It uses **the same input** as `DawnHardeningTests` — seeing whether both backends keep the same contract requires that.
final class MetalHardeningTests: XCTestCase {
